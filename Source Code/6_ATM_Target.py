...
def topik():
	...
	with open('atm_target.csv') as raw:
	...
	fhandler = logging.FileHandler(filename='atmtarget-' + str(jmltopik) + '-logging.csv', mode='w')
	...
	df_banyak.to_csv('atmtarget-' + str(jmltopik) + '-coherence.csv', index=False)
	...
	atm1.save('atmtarget-' + str(jmltopik) + '-topik.atmodel')

	with open('atmtarget-' + str(jmltopik) + '-topik-jumlahkata.csv','w') as tulisFile:
	...
	with open('atmtarget-' + str(jmltopik) + '-topik-detail.csv','w') as tulisFile:
	    tulisFileWriter = csv.writer(tulisFile,lineterminator='\n')
	    d=0
	...
	with open('atmtarget-' + str(jmltopik) + 'probabilitas.csv','w') as tulisFile:
	    tulisFileWriter = csv.writer(tulisFile,lineterminator='\n')
	    initopik = 0
	    i=0
	...
	lines = [line.strip() for line in open('atmtarget-' + str(jmltopik) + '-logging.csv')]

	for x in lines:
	    try:
	        found = re.search('per-word bound, (.+?) perplexity', x).group(1)
	        f = open('atmtarget-' + str(jmltopik) + '-perplexity.csv', 'a+')
	        f.write("\n"+found)
